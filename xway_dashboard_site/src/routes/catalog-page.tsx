...
const CATALOG_CHART_SOURCE_CACHE_STORAGE_PREFIX = "xway-catalog-chart-source-cache-v1";
...
const CATALOG_SUMMARY_METRICS_SETTINGS_VERSION = 2;
...
const DEFAULT_CATALOG_SUMMARY_METRIC_SETTINGS: CatalogSummaryMetricSettings = {
  ...
  drr: { visible: true, compact: false },
...
function mergeCatalogPayloadWithShopResponse(basePayload: CatalogResponse, shopResponse: CatalogResponse) {
  ...
  shops.sort((left, right) => `${left.name || ""}::${left.id}`.localeCompare(`${right.name || ""}::${right.id}`, "ru"));
  const totals = computeCatalogTotals(shops);
  return {
    ...basePayload,
    generated_at: shopResponse.generated_at || basePayload.generated_at,
    range: shopResponse.range || basePayload.range,
    total_shops: shops.length,
    total_articles: shops.reduce((sum, shop) => sum + shop.articles.length, 0),
    totals,
    shops,
  };
}
...
function catalogCampaignTypeHasMetrics(totals: CatalogCampaignTypeTotals | null | undefined) {
  if (!totals) {
    return false;
  }
  return ["views", "clicks", "atbs", "orders", "spend", "revenue"].some((field) => {
    const value = totals[field as keyof CatalogCampaignTypeTotals];
    return typeof value === "number" && Number.isFinite(value) && value > 0;
  });
}

function resolveVisibleCatalogCampaignTypeRows(article: CatalogArticle, totalsByType: CatalogCampaignTypeTotalsByKey | null | undefined) {
  const hasCpcSlot = article.campaign_states.some((state) => state.key === "cpc");
  const hasCpcMetrics = catalogCampaignTypeHasMetrics(totalsByType?.cpc);
  return CATALOG_CAMPAIGN_TYPE_ROWS.filter((type) => type.key !== "cpc" || hasCpcSlot || hasCpcMetrics);
}
...
function CatalogCampaignTypeRowsLegend({ visibleTypes }: { visibleTypes: typeof CATALOG_CAMPAIGN_TYPE_ROWS }) {
  return (
...
function CatalogCampaignTypeMetricStack({
  ...
  visibleTypes: typeof CATALOG_CAMPAIGN_TYPE_ROWS;
...
function readCatalogSummaryMetricSettings(): CatalogSummaryMetricSettings {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as Partial<Record<CatalogSummaryMetricKey, Partial<CatalogSummaryMetricSetting>>> & { __version?: number };
    const shouldApplyNewDrrDefault = Number(parsed.__version ?? 1) < CATALOG_SUMMARY_METRICS_SETTINGS_VERSION;
    return CATALOG_SUMMARY_METRIC_KEYS.reduce((settings, key) => {
      const defaultSetting = DEFAULT_CATALOG_SUMMARY_METRIC_SETTINGS[key];
      settings[key] = {
        visible:
          key === "drr" && shouldApplyNewDrrDefault
            ? true
            : typeof parsed[key]?.visible === "boolean"
              ? Boolean(parsed[key]?.visible)
              : defaultSetting.visible,
...
function catalogChartSourcePersistentCacheKey(cacheKey: string) {
  return `${CATALOG_CHART_SOURCE_CACHE_STORAGE_PREFIX}:${cacheKey}`;
}

function isCompleteCatalogChartResponse(response: CatalogChartResponse | null | undefined) {
  if (!response) {
    return false;
  }
...
function isCompleteCatalogChartCacheEntry(entry: CatalogChartCacheEntry | null | undefined): entry is CatalogChartCacheEntry {
  if (!entry) {
    return false;
  }
  return isCompleteCatalogChartResponse(entry.response);
}

function readCatalogChartSourceCache(cacheKey: string) {
  return readPersistentApiCache<CatalogChartCacheEntry>(catalogChartSourcePersistentCacheKey(cacheKey));
}
...
function summarizeCatalogArticles(articles: CatalogArticle[]) {
  return articles.reduce(
    (totals, article) => {
      ...
      totals.sum_price += toNumber(article.sum_price) ?? 0;
      return totals;
    },
    {
      views: 0,
...
function catalogArticleHasCampaignSignals(article: CatalogArticle | null | undefined) {
  if (!article || isCatalogArticleDisabled(article)) {
    return false;
  }
  return (
    article.campaign_states.length > 0 ||
    (toNumber(article.campaigns_count) ?? 0) > 0 ||
    (toNumber(article.manual_campaigns_count) ?? 0) > 0 ||
    (toNumber(article.expense_sum) ?? 0) > 0
  );
}

function catalogArticleHasOrderSignals(article: CatalogArticle | null | undefined) {
  if (!article || isCatalogArticleDisabled(article)) {
    return false;
  }
  return (
    (toNumber(article.orders) ?? 0) > 0 ||
    (toNumber(article.ordered_report) ?? 0) > 0 ||
    (toNumber(article.ordered_sum_report) ?? 0) > 0 ||
    (toNumber(article.sum_price) ?? 0) > 0
  );
}
...
export function CatalogPage() {
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem(
      CATALOG_SUMMARY_METRICS_SETTINGS_STORAGE_KEY,
      JSON.stringify({ __version: CATALOG_SUMMARY_METRICS_SETTINGS_VERSION, ...summaryMetricSettings }),
    );
    ...
    const timer = window.setTimeout(() => {
      void (async () => {
        ...
        if (
          ...
          setChartLoading(false);
          setChartError(null);
          setChartSourceData(persistedResponse);
          setChartProgress({
            cacheKey: chartSourceCacheKey,
            selectionCount: persistedResponse.selection_count,
            loadedProductsCount: persistedResponse.loaded_products_count,
            chunkCount: 0,
            loadedChunkCount: 0,
            errorCount: persistedResponse.errors.length,
          });
          return;
        }
  ...
  const catalogChartRows = chartData?.rows ?? [];
  const chartSparklineRows = catalogChartRows.slice(-7);
  const previousChartSparklineRows = catalogChartRows.slice(-14, -7);
  const chartRowsRate = (rows: CatalogChartResponse["rows"]) =>
    catalogChartRate(
      rows.reduce((total, row) => total + (toNumber(row.expense_sum) ?? 0), 0),
      rows.reduce((total, row) => total + (toNumber(row.sum_price) ?? 0), 0),
    );
  ...
  const drr = chartSparklineRows.length ? chartRowsRate(chartSparklineRows) : catalogChartRate(toNumber(visibleTotals.expense_sum) ?? 0, toNumber(revenue) ?? 0);
  ...
  const compareDrr = previousChartSparklineRows.length
    ? chartRowsRate(previousChartSparklineRows)
    : compareVisibleTotals
      ? catalogChartRate(toNumber(compareVisibleTotals.expense_sum) ?? 0, toNumber(compareRevenue) ?? 0)
      : null;
  ...
  const buildSummarySparkline = (
    ...
    if (chartSparklineRows.length < 2) {
      return undefined;
    }
  ...
  const mergeChartResponseForRefs = async (
    ...
    setChartSourceData((current) => {
      const baseResponse = isCompleteCatalogChartResponse(current) ? current : (persistedBase ?? current);
      const nextResponse = baseResponse
        ? mergeCatalogChartRetryResponse(baseResponse, response, chartSourceSelectionCount, productRefs)
  ...
  const refreshCatalogProductRefs = async (
    ...
    try {
      ...
      for (let chartProcessIndex = 0; chartProcessIndex < chartProcessGroups.length; chartProcessIndex += 1) {
        ...
        try {
          const deferCampaignBreakdown = options.campaignChartMode === "deferred";
          const hasPreloadedAbsoluteChartRows = chartProcessRefs.every((ref) => options.knownChartRowsByRef?.has(ref));
          if (deferCampaignBreakdown && hasPreloadedAbsoluteChartRows) {
            appendCatalogRefreshLogsForRefs(chartProcessRefs, {
              status: "info",
              scope,
              step: "Данные РК: графики",
              message: "Точная РК-разбивка отложена",
              detail: "Абсолютный дневной график уже взят из пакетной загрузки кабинета; точная дневная РК-разбивка загрузится при раскрытии товара или точечном обновлении.",
            });
            ...
            if (expandedRefs.length) {
              setArticleAnalyticsByRef((current) => {
                ...
                expandedRefs.forEach((ref) => {
                  const productRows = options.knownChartRowsByRef?.get(ref) ?? [];
          ...
          try {
            chartResponse = await fetchCatalogChartWorkerSafe({
              ...
              includeCampaignTypes: !deferCampaignBreakdown,
              forceRefresh: true,
              signal: options.signal,
              selectionCount: chartSourceSelectionCount,
              chunkSize: deferCampaignBreakdown ? CATALOG_CHART_FETCH_CHUNK_SIZE : CATALOG_CAMPAIGN_TYPE_FETCH_CHUNK_SIZE,
              onWorkerLimit: (limitRefs) => {
            ...
            chartHasCampaignTypeBreakdown = !deferCampaignBreakdown;
          } catch (error) {
  ...
  const fetchCatalogIssueRowsForRefs = async (
    ...
    const fetchIssueRowsChunk = async (chunkRefs: string[]): Promise<CatalogIssueCacheEntry[]> => {
      let cursor: string | null = null;
      const chunkRows: CatalogIssueCacheEntry[] = [];
      do {
        const response = await fetchCatalogIssues({
          productRefs: chunkRefs,
          start: yesterdayIso,
          ...
          cursor,
          limitProducts: Math.min(CATALOG_ISSUES_PAGE_SIZE, Math.max(chunkRefs.length, 1)),
          deadlineMs: CATALOG_ISSUES_WORKER_DEADLINE_MS,
          forceRefresh: options.forceRefresh,
          signal,
        });
        chunkRows.push(...response.rows.map(mapCatalogIssuesRowToCacheEntry));
        cursor = response.next_cursor ?? null;
        if (cursor && !signal.aborted) {
          await waitForCatalogIssuesRetry(Math.min(CATALOG_ISSUES_RETRY_DELAY_MS, 250), signal);
        }
      } while (cursor && !signal.aborted);
      return chunkRows;
    };

    const fetchIssueRowsAdaptive = async (chunkRefs: string[]): Promise<CatalogIssueFetchResult> => {
      try {
        const chunkRows = await fetchIssueRowsChunk(chunkRefs);
        const loadedRefs = [...new Set(chunkRows.map((row) => row.product_ref))];
        return {
          rows: chunkRows,
          loadedRefs,
          failedRefs: chunkRefs.filter((ref) => !loadedRefs.includes(ref)),
        };
      } catch (error) {
        if (chunkRefs.length > 1 && (await isWorkerSubrequestLimitError(error))) {
          const middle = Math.ceil(chunkRefs.length / 2);
          const left = await fetchIssueRowsAdaptive(chunkRefs.slice(0, middle));
          const right = await fetchIssueRowsAdaptive(chunkRefs.slice(middle));
          return {
            rows: [...left.rows, ...right.rows],
            loadedRefs: [...left.loadedRefs, ...right.loadedRefs],
            failedRefs: [...left.failedRefs, ...right.failedRefs],
        ...
        if (!signal.aborted) {
          errorMessages.add(await readApiErrorMessage(error));
        }
        return {
          rows: [],
          loadedRefs: [],
          failedRefs: chunkRefs,
        };
      }
    };
    ...
    const result = {
      rows,
      loadedRefs,
      failedRefs: pendingRefs,
    };
    if (errorMessages.size) {
      Object.defineProperty(result, "errorMessages", {
        value: [...errorMessages],
        enumerable: false,
      });
    }
  ...
  const refreshCatalogIssuesForRefsBatch = async (
    productRefs: string[],
    options: {
      signal: AbortSignal;
      scope: CatalogRefreshLogScope;
    ...
    if (result.loadedRefs.length) {
      ...
      appendCatalogRefreshLogsForRefs(result.loadedRefs, {
        status: "success",
        scope: options.scope,
        step: "Ошибки за вчера",
        message: "Проверено пакетно",
      }, (ref) => {
        const issueCount = rowsByRef.get(ref)?.issues.length ?? 0;
        return {
          message: issueCount > 0 ? `Найдено ошибок: ${formatNumber(issueCount)}` : "Ошибок не найдено",
        };
      });
    ...
    if (result.failedRefs.length) {
      appendCatalogRefreshLogsForRefs(result.failedRefs, {
        status: "warning",
        scope: options.scope,
        step: "Ошибки за вчера",
        message: "Не проверено в пакетном режиме",
        detail: "Источник не успел вернуть эти товары в лимит Worker; можно догрузить ошибки отдельной кнопкой.",
      });
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-5">
        {visibleShops.map((shop) => {
          ...
          const previousShopSparklineRows = (shopChartData?.rows ?? []).slice(-14, -7);
          const shopDrr = shopSparklineRows.length ? chartRowsRate(shopSparklineRows) : catalogChartRate(totals.expense_sum, totals.sum_price);
          const compareShopDrr = previousShopSparklineRows.length
            ? chartRowsRate(previousShopSparklineRows)
            : compareShopTotals
              ? catalogChartRate(compareShopTotals.expense_sum, compareShopTotals.sum_price)
              : null;
          ...
          const shopMetrics: Array<{ label: string; value: string; deltaText?: ReactNode; deltaClassName: string; sparkline?: CatalogSummarySparkline }> = [
            ...
            {
              label: "ДРР 7 дней",
              value: formatPercent(shopDrr),
              deltaText: renderDeltaText(formatSignedPercent(diffValue(shopDrr, compareShopDrr))),
              deltaClassName: deltaClassName(diffValue(shopDrr, compareShopDrr), false),
              sparkline: buildShopSparkline("drr", (row) => row.drr_ads),
            },
            {
              label: "Баланс",
                    ...
                    columns={[
                      ...
                      },
                      {
                        ...
                        render: (article) => {
                          ...
                          return <CatalogCampaignTypeRowsLegend visibleTypes={resolveVisibleCatalogCampaignTypeRows(article, totalsByType)} />;
                        },
                      },
                      {
                        key: "spend",
