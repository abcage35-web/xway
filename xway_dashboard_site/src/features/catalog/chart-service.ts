...
export function mergeCatalogChartResponses(
  ...
  return {
    ...
    processed_products_count: (current?.processed_products_count ?? 0) + (incoming.processed_products_count ?? incoming.loaded_products_count ?? 0),
    remaining_products_count: incoming.remaining_products_count ?? current?.remaining_products_count,
    complete: incoming.complete ?? current?.complete ?? true,
    next_cursor: incoming.next_cursor ?? null,
    requested_products: incoming.requested_products ?? current?.requested_products,
    rows,
